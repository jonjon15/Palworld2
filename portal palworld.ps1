# Script de setup do Palworld Admin Panel para Windows PowerShell
# Execute: .\setup-project.ps1

Write-Host "🚀 Criando estrutura do projeto Palworld Admin Panel..." -ForegroundColor Green

# Criar estrutura de pastas
New-Item -ItemType Directory -Force -Path "app\api\server\info" | Out-Null
New-Item -ItemType Directory -Force -Path "app\api\players" | Out-Null
New-Item -ItemType Directory -Force -Path "app\api\pals\spawn" | Out-Null
...
New-Item -ItemType Directory -Force -Path "public\map" | Out-Null
...
'@ | Out-File -FilePath "types\palworld.ts" -Encoding UTF8
...
'@ | Out-File -FilePath "services\palworldApiClient.ts" -Encoding UTF8
...
'@ | Out-File -FilePath "services\rconClient.ts" -Encoding UTF8
...
'@ | Out-File -FilePath "app\components\Map.tsx" -Encoding UTF8
...
'@ | Out-File -FilePath "app\components\SpawnModal.tsx" -Encoding UTF8
...
'@ | Out-File -FilePath "app\api\server\info\route.ts" -Encoding UTF8
...
'@ | Out-File -FilePath "app\api\players\route.ts" -Encoding UTF8
...
'@ | Out-File -FilePath "app\api\pals\spawn\route.ts" -Encoding UTF8

# README
@'
# Palworld Admin Panel

Painel web para administração de servidores Palworld.
...
Edite .env.local com suas credenciais do servidor.
...
Write-Host "3. Edite .env.local com suas credenciais"
